...
# Cache tween reference to avoid creating multiple tweens
...
func _update_bar(current_health):
	...
	#這個是用於緩衝條的動畫 - optimized to kill existing tween first
	if catchup_tween:
		catchup_tween.kill()
	catchup_tween = create_tween()
	catchup_tween.tween_property(catchup_bar, "value", percentage, 0.4).set_trans(Tween.TRANS_SINE)
