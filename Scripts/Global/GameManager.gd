...
#敵人生成數量，初始值為5，根據DestinyManager的enemy_quantity_multiplier進行修改

var enemy_spawn_quantity : int = 5
...
func load_game():
	...
	if file:
		...
		if parse_result == OK:
			...
			PlayerData.gold_quantity = pd_data.get("gold_quantity", 20) + save_data["world"].get("room_gold_quantity", 0) # 從存檔中還原玩家金幣數量，並加上當前房間內的金幣數量
